...
#compute error for ank_lw using tree method, then boost method, then rf method and take differences
set.seed(98815)
a<-AL_trees(train_ank_hip,test_ank_lw)
c<-AL_bagging(na.omit(train_ank_lw),na.omit(test_ank_hip),52)
d<-AL_rf(na.omit(train_ank_lw),na.omit(test_ank_hip),52)
xt_a<-c(a)
xt_dc<-c(a-c)
xt_dd<-c(a-d)
...
for(i in 2:1000){
  ...
  c<-AL_bagging(na.omit(train2a),na.omit(test_ank_lw),52)
  d<-AL_rf(na.omit(train2a),na.omit(test_ank_lw),52)
  xt_a[i]<-c(a)
  xt_dc[i]<-c(a-c)
  xt_dd[i]<-c(a-d)
}
...
c<-AL_bagging(na.omit(train2_ank_lw),na.omit(test2_ank_lw),52)
d<-AL_rf(na.omit(train2_ank_lw),na.omit(test2_ank_lw),52)
xt2_a<-c(a)
xt2_dc<-c(a-c)
xt2_dd<-c(a-d)
...
#xt_a errors for tree method for ank_lw location, #xt_dc differences for tree method against bagging method for ank_lw location, 
#xt_dd differences for tree method against rf method for ank_lw location (binary with a 2)
