...
#Read data with just ank_hip location. 
...
#compute error for ank_hip using tree method and then boost method and take difference
set.seed(98815)
a<-AL_trees(train_ank_hip,test_ank_hip)
b<-boost_tree(na.omit(train_ank_hip),na.omit(test_ank_hip),53,80)
xt_a<-c(a)
xt_db<-c(a-b)
...
for(i in 2:1000){
  ...
  b<-boost_tree(na.omit(train2a),na.omit(test_ank_hip),53,80)
  xt_a[i]<-c(a)
  xt_db[i]<-c(a-b)
}
...
b<-boost_tree(na.omit(train2_ank_hip),na.omit(test2_ank_hip),53,80)
xt2_a<-c(a)
xt2_db<-c(a-b)
#start loop. take random sample from training sets with same index. Compute tree errors and boost errors store differences.
for(i in 2:1000){
  ...
  b<-boost_tree(na.omit(train2a),na.omit(test2_ank_hip),53,80)
  xt2_a[i]<-c(a)
  xt2_db[i]<-c(a-b)
}
#end of loops for two classes

#xt_a errors for tree method for ank_hip locations, #xt_db differences for tree method against boost method for ank_hip locations (binary with a 2)
save(xt_a,xt_db,xt2_a,xt2_db,file = "ALank_hipdiff1.RData")