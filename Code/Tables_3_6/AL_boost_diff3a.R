...
d<-boost_tree(train_nomiss_lw,test_nomiss_lw,77,80)
e<-boost_tree(train_nomiss_rw,test_nomiss_rw,77,80)
xt_a<-c(a)
xt_dd<-c(a-d)
xt_de<-c(a-e)
...
for(i in 2:1200){
  ...
  d<-boost_tree(train2d,test_nomiss_lw,77,80)
  e<-boost_tree(train2e,test_nomiss_rw,77,80)
  xt_a[i]<-a
  xt_dd[i]<-c(a-d)
  xt_de[i]<-c(a-e)
}
...
d<-boost_tree(train2_nomiss_lw,test2_nomiss_lw,77,80)
e<-boost_tree(train2_nomiss_rw,test2_nomiss_rw,77,80)
xt2_a<-c(a)
xt2_dd<-c(a-d)
xt2_de<-c(a-e)
#start loop. take random sample from training sets with same index. Compute bagging errors and store differences.
for(i in 2:1200){
  ...
  d<-boost_tree(train2d,test2_nomiss_lw,77,80)
  e<-boost_tree(train2e,test2_nomiss_rw,77,80)
  xt2_a[i]<-a
  xt2_dd[i]<-c(a-d)
  xt2_de[i]<-c(a-e)
}
...
save(xt_a,xt_dd,xt_de,xt2_a,xt2_dd,xt2_de,file = "ALboostdiff3a.RData")
